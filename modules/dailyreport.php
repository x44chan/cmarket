...
<div class="container">
	...
	<form action="" method = "post" class="form-group">
		...
		<table style = "margin-top: -10px;" class="table" id = "table" align="left">
			...
			<tr>
				<td colspan="12" align="center"> <i><label id = "err" style="color: red; display: none;">Select atleast 1 collection type.</label></td>
			</tr>
		</table>
...
<?php 
	if($result->num_rows > 0){
		...
		while ($row = $result->fetch_assoc()) {
			...
			if($row['type'] == "Market Fee"){
				$mfeeamount = '₱ ' . number_format($row['amount']);
				$mfeecov = strtoupper($month);
				$mfeetotal += $row['amount'];
			}elseif($row['type'] == "Electric Bill"){
				$ebill = '₱ ' . number_format($row['amount']);
				$ecov = strtoupper($month);
			}elseif($row['type'] == "Water Bill"){
				$wbill = '₱ ' . number_format($row['amount']);
				$wcov = strtoupper($month);
			}elseif($row['type'] == "Market Clearance"){
				$mc = '₱ ' . number_format($row['amount']);
			}elseif($row['type'] == "Business Tax" || $row['type'] == "Space Rental"){
				$btsr = '₱ ' . number_format($row['amount']);
			}elseif($row['type'] == "Anti Mortem"){
				$am = '₱ ' . number_format($row['amount']);
			}elseif($row['type'] == "Post Mortem"){
				$pm = '₱ ' . number_format($row['amount']);
			}elseif($row['type'] == "Transfer Fee"){
				$tf = '₱ ' . number_format($row['amount']);
			}elseif($row['type'] == "Renewal Fee"){
				$rf = '₱ ' . number_format($row['amount']);
			}elseif($row['type'] == "Goodwill"){
				$g = '₱ ' . number_format($row['amount']);
			}elseif($row['type'] == "TCT"){
				$tct = '₱ ' . number_format($row['amount']);
			}
			echo '<tr>';
